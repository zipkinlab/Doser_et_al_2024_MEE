...
sp.names <- dimnames(neonDWP$y)[[1]]
sp.names.factor <- factor(sp.names,
		         levels = sp.names)
...
det.comm.quants <- quantile(exp(out$alpha.comm.samples[, 1]), c(0.025, 0.975))
...
det.plot <- ggplot(data = comm.plot.df) + 
  ...
  theme(text = element_text(family = 'LM Roman 10')) +
  labs(x = 'Distance (m)', y = 'Detection Probability', col = 'Species', title = '(B)')
